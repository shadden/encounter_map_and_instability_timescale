...
if __name__=="__main__":
    ...
    Niter = 3000 # number of iterations for numerical measurement of diffusion rate
    ...
    # Add point (q,Ncrit)=(35,4) to KAM crit data
    q_kam = np.concatenate(([30,35],q_kam))
    Ncrit = np.concatenate(([0,4],Ncrit))

    Nmin = int(interp1d(q_kam/30,Ncrit)(q)) + 10 # min MMR
    Nmax = Nmin+400 # max MMR
    ...
    with Pool(processes=NCPU) as pool:
        answer = pool.map(runme,pars)
    print("parallel eval: {}".format(time.time() - t))
